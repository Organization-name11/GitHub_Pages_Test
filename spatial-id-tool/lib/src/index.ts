...
export class Space {
  ...
  up(by: number = 1) { return this.move({ f: by }); }
  down(by: number = 1) { return this.move({ f: -by }); }
  north(by: number = 1) { return this.move({ y: by }); }
  south(by: number = 1) { return this.move({ y: -by }); }
  east(by: number = 1) { return this.move({ x: by }); }
  west(by: number = 1) { return this.move({ x: -by }); }
  ...
  surroundings(): Space[] {
    return [
      ...
      ...getSurrounding(this.up().zfxy).map((tile) => new Space(tile)),
      ...getSurrounding(this.down().zfxy).map((tile) => new Space(tile)),
    ];
  }
  ...
  contains(position: LngLatWithAltitude) {
    ...
    return (
      ...
      (typeof position.alt !== 'undefined' ? position.alt >= floor && position.alt < ceil : true)
    );
  }
  ...
  static getSpaceById(id: string, zoom?: number) { return new Space(id, zoom); }
  static getSpaceByLocation(loc: LngLatWithAltitude, zoom?: number) { return new Space(loc, zoom); }
  static getSpaceByZFXY(zfxyStr: string) { return new Space(zfxyStr); }
  ...
  static spacesForGeometry(geom: Geometry, zoom: number): Space[] {
    const z = zoom;
    if (z === 0) return [new Space('0/0/0/0')];
    if (geom.type === 'GeometryCollection') throw new Error('GeometryCollection not supported');
    ...
    for (let x = minX; x <= maxX; x++) {
      for (let y = minY; y <= maxY; y++) {
        ...
        if (turfBooleanIntersects(geom, space.toGeoJSON())) spaces.push(space);
      }
    }
    return spaces;
  }
